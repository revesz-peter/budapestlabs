...
export interface DesertSandShadersProps extends React.HTMLAttributes<HTMLDivElement> {
  /**
  ...
  /**
   * Sand texture detail level
   * @default 1.0
   */
  sandDetail?: number

  /**
   * Surface bump mapping intensity
   * @default 1.0
   */
  bumpIntensity?: number

  /**
   * Atmospheric mist density
   * @default 1.0
   */
...
vec3 normal(in vec3 p, float ef) {
    ...
    return normalize(vec3(map(p + e.xyy*ef) - map(p - e.xyy*ef),
                          map(p + e.yxy*ef) - map(p - e.yxy*ef),
                          map(p + e.yyx*ef) - map(p - e.yyx*ef)));
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    vec3 ro = vec3(0, 1.2, iTime*2.*u_speed);
    ...
    if (t < FAR){
        ...
        sn = doBumpMap(sp, sn, .07*u_bumpIntensity);
        ...
        col = mix(vec3(1, .95, .7), vec3(.9, .6, .4), fBm(sp.xz*16.*u_sandDetail));
        col = mix(col*1.4, col*.6, fBm(sp.xz*32.*u_sandDetail - .5));
    ...
    col = col*.75 + (col + .25*vec3(1.2, 1, .9))*mistCol*dust*1.5*u_mistIntensity;
...
export const DesertSandShaders = forwardRef<HTMLDivElement, DesertSandShadersProps>(
  (
    {
      className,
      speed = 1.0,
      ...
      <div className={cn("w-full h-full", className)} ref={ref} {...(props as any)}>
        <Shader
          ...
          uniforms={{
            ...
            u_sandDetail: { type: "1f", value: sandDetail },
            u_bumpIntensity: { type: "1f", value: bumpIntensity },
            u_mistIntensity: { type: "1f", value: mistIntensity },
          }}
        />
      </div>
    )
...
DesertSandShaders.displayName = "DesertSandShaders"

export default DesertSandShaders
